...
namespace Service.Services
{
    public class CityService : ICityService
    {
        ...
        public async Task DeleteAsync(int? id)
        {
            ...
            await _cityRepo.DeleteAsync(existCity);
        }
        ...
        public async Task EditAsync(int? id, CityEditDto model)
        {
            ...
            var existCity = await _cityRepo.GetById((int)id) ?? throw new NullReferenceException();

            _mapper.Map(model, existCity);

            await _cityRepo.EditAsync(existCity);
        }
